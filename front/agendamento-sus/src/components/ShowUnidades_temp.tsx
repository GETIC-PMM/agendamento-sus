...
import { DaysInterface, Secretaries, Unidade } from '../interfaces/interfaces';
import { useAppointmentTypes } from '../api/routes/appointment-type-api';
import { useUnitAppointmentsById } from '../api/routes/appointments-api';

const ShowUnidades = () => {
  const [unidadeData, setUnidadeData] = useState<Unidade[]>([]);
  const [tiposAtendimento, setTiposAtendimento] = useState<any[]>([]);
  const [unitAppointments, setUnitAppointments] = useState<Secretaries[]>([]);

  const [isAnyUnitWithoutAppointmentData, setIsAnyUnitWithoutAppointmentData] =
    useState<boolean>(false);

  const [selectedTipoAtendimento, setSelectedTipoAtendimento] =
    useState<any>(null);
  const [, setSelectedWorkingDays] = useState<string[]>([]);

  const [unitsIsLoading, setUnitsIsLoading] = useState<boolean>(true);
  const [unitAppointmentsIsLoading, setUnitAppointmentsIsLoading] =
  ...
  //REQUISIÇÕES
  const appointmentTypesQuery = useAppointmentTypes(); // Tipos de atendimento
  const { data: appointmentTypes, isLoading: appointmentTypesIsLoading } =
    appointmentTypesQuery;

  const [selectedUnit, setSelectedUnit] = useState<string>('');
  const unitAppointmentsQuery = useUnitAppointmentsById(selectedUnit);
  const {
    data: unitAppointmentsData,
    isLoading: unitAppointmentsDataIsLoading,
  } = unitAppointmentsQuery;
  ...
  const submiteditUnitId = async () => {
    ...
    await axios
      .post(
        'http://localhost:8000/api/secretaries',
        {
          unit_id: editUnitId,
  ...
  const getUnitAppointments = async (unitId: number) => {
    await axios
      .get(`http://localhost:8000/api/secretaries/appointment_type/${unitId}`, {
        headers: {
          Authorization: `Bearer ${Cookies.get('token')}`,
        },
      })
      .then(response => {
        ...
        console.log('UNIT APPOINTMENTS', response.data.data);
      });
  };

  const handleDeleteSecretarie = async (
    unitId: number,
    appointment_type_id: number,
  ) => {
    await axios
      .delete(
        `http://localhost:8000/api/secretaries/${unitId}/${appointment_type_id}`,
        {
          headers: {
            Authorization: `Bearer ${Cookies.get('token')}`,
          },
  ...
  return (
    <>
      ...
      >
        <div className="py-6 w-3/4 px-6 rounded  flex flex-col bg-zinc-100">
          <span className="text-lg font-bold text-center text-primary-base">
            Editar dados de agendamento
          </span>
          <div className="flex flex-col mt-4 w-full">
            ...
            {unitAppointmentsIsLoading ? (
              ...
              unitAppointments?.map(appointment => {
                const appointment_name = appointmentTypes?.data.find(
                  tipo => tipo.id === appointment.appointment_type_id,
                )?.name;

                return (
                  <div
                    key={appointment.appointment_type_id}
                    className="flex flex-col mb-4  border-zinc-400 rounded py-2"
                  >
              ...
              <>
                <div className="bg-red-error mb-3 rounded py-4 text-white text-sm text-center">
                  <span className="font-bold">
                    Não existem tipos de atendimentos cadastrados para essa
                    unidade.
                  </span>
                </div>
              </>
            )}
            <h1 className="my-2 text-primary-base font-bold text-center">
              Adicionar novo tipo de atendimento
            </h1>
            <Select
              value={selectedTipoAtendimento}
              onChange={e => {
                setSelectedTipoAtendimento(e.target.value);
              }}
            >
              {appointmentTypes?.data.map(tipo => (
        ...
        <>
          <div className="bg-red-error mb-3 rounded p-2 text-white text-sm text-center w-full">
            <span className="block font-bold">
              Sempre confira se existem unidades sem dados de agendamento
              definidos.
            </span>
            <span className="block">
              Defina os dados de agendamento pressionando o icone{' '}
              <span>
                <FiEdit style={{ display: 'unset' }} />
              </span>
            </span>
          </div>
                ...
                {unidadeData.map((row: Unidade) => (
                  ...
                  >
                    <>
                      ...
                      <TableCell align="center">
                        {dayjs(row.open_time).format('HH:mm')} as{' '}
                        {dayjs(row.close_time).format('HH:mm')}
                      </TableCell>
                      <TableCell>
